...
library(ggsignif)    # barre di significativitÃÂ 
...
# Boxplot pulito con jitter e barre di significativitÃÂ  Wilcoxon - Figura 4
ggplot(msi_alti, aes(x = Tumore, y = MSI, color = Tumore)) +
  ...
  labs(
    title = "Distribuzione MSI per tipo di tumore (MSI > 20) con significativita'",
    x = "Tipo di tumore",
    y = "MSI"
  ) +
  theme_minimal() +
...
# ---------------------------- LIBRERIE AGGIUNTIVE ------------------------ 
# Assicurati di aver giÃÂ  caricato:
library(dplyr)
library(ggplot2)
library(ggsignif)
...
# ---------------------------- 2. TEST SHAPIRO-WILK ------------------------ 
# Verifica formalmente la normalitÃÂ  per ciascun tumore - Figura 5
shapiro_results <- by(msi_alti$MSI, msi_alti$Tumore, shapiro.test)
shapiro_results
# Nota: p-value < 0.05 indica deviazione dalla normalitÃÂ 
...
# Con correzione Bonferroni per confronti multipli - Figura 13
...
# ----------------------------
# 2. Dataset clinico pulito con MSI numerico e TEST DI FISHER: associazione MSI-H vs BRCA1/2 - Figura 19
# ----------------------------
...
# Pazienti con varianti BRCA1/2
brca_status <- data_mutations_extended %>%
  filter(V2 %in% c("BRCA1","BRCA2")) %>%
  distinct(V1) %>%
  mutate(BRCA_status = "Mutato") %>%
  rename(Paziente_ID = V1)

# Unione con dataset clinico
fisher_data <- data_clean %>%
  select(Paziente_ID, Stato_MSI) %>%
  left_join(brca_status, by = "Paziente_ID") %>%
  mutate(
    BRCA_status = ifelse(is.na(BRCA_status), "WT", "Mutato")
  )

# Tabella di contingenza
tabella_fisher <- table(fisher_data$Stato_MSI, fisher_data$BRCA_status)

print(tabella_fisher)

# Test esatto di Fisher
fisher_result <- fisher.test(tabella_fisher)
...
# ----------------------------
# 10. Visualizzazione grafica mutazioni MMR - Figura 22
# ----------------------------
...
cat("ÃÂ¢ÃÂÃÂ Varianti filtrate:", nrow(filtered_final), "\n")

# ---------------------------
# 4. Rimozione varianti note giÃÂ  analizzate - Figura 23
# ---------------------------
...
cat("ÃÂ¢ÃÂÃÂ Righe totali nel dataset filtrato (senza varianti note):", nrow(dataset_filtrato), "\n")
...
# ---------------------------
# 8. Grafico barre normalizzato con percentuali - Figura 24
# ---------------------------
...
# ---------------------------
# 9. Elenco pazienti con varianti novel - Figura 25
# ---------------------------
...
# ============================================================
# SESTA PARTE: GENERAZIONE TABELLE: Figure 14, 16, 18, 20, 21
# ============================================================
...
# ----------------------------
# 4. FIGURA 20 - lista pazienti MSI > 20
# ----------------------------
...
# ----------------------------
# 5. FIGURA 21 - lista pazienti MSI < 20
# ----------------------------
...
# ----------------------------
# 3. Test statistico non parametrico â Wilcoxon - Figura 8
# ----------------------------
